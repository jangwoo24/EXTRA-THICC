...
def main():
    ...
    parser.add_argument('--input_str', type=str, default='Hello',
                       help='input string')
    ...
    #print("before " + args.input_str)
    args.input_str = args.input_str.replace("&" , " ");
    #print("after " + args.input_str)
    x = sample_main(args)
    #print ("Returned from sample_main)")
    print(x)
    return[x]
...
def sample_main(args):
    ...
    with tf.Session(config=config) as sess:
        ...
        x = chatbot(net, sess, chars, vocab, args.n, args.beam_width,
                args.relevance, args.temperature, args.topn,args)
        if len(args.input_str) > 0: return[x]
...
def chatbot(net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn, args):
    x = ""
    states = initial_state_with_relevance_masking(net, sess, relevance)
    while True:
        if len(args.input_str) > 0:
            user_input = args.input_str
